...
#define SRUTIL_DELEGATE_INVOKER_CLASS_NAME SRUTIL_DELEGATE_JOIN_MACRO(delegate_invoker,SRUTIL_DELEGATE_PARAM_COUNT)
	...
	public:
		...
		typedef SRUTIL_DELEGATE_INVOKER_CLASS_NAME<return_type(SRUTIL_DELEGATE_TEMPLATE_ARGS)> invoker_type;
#else
		...
		public:
			...
			SRUTIL_DELEGATE_CLASS_NAME operator&() {return from_method<proxy, &proxy::invoke>(this);}

		private:
			return_type invoke(SRUTIL_DELEGATE_PARAMS) {f(SRUTIL_DELEGATE_ARGS);}
		};
