...
namespace </xsl:text>
    ...
    <xsl:call-template name="print-constants"/>
...
<xsl:key name="k" match="*//option/@name" use="self::node()"/>
...
<xsl:template name="print-constants">
    ...
    <xsl:variable name="max-len">
        <xsl:choose>
            <xsl:when test="$max-name-len+3 > $max-val-len"><xsl:value-of select="$max-name-len+3"/></xsl:when>
    ...
    <xsl:for-each select="(*//option/@name)[count(.|key('k', self::node())[1]) = 1]">
