...
::Wait(int a_timeout_msec)
{
  ...
  while((rc = epoll_wait(m_epoll_fd,cev,N,a_timeout_msec)) < 0 && errno==EINTR);
  ...
  for (epoll_event* p = cev, *end = cev + rc; p != end; ++p) {
    ...
    try {
      if (UNLIKELY(IsError(p->events))) {
        ...
        // This is normal READ/WRITE notification => invoke associated handler:
