...
***** END LICENSE BLOCK *****
*/
#ifndef _UTXX_SHARED_PTR_HPP_
#define _UTXX_SHARED_PTR_HPP_
...
struct shared_ptr
{
    ...
    bool operator!=(const shared_ptr& a_rhs) const noexcept { return *this  != a_rhs;   }
    bool operator< (const shared_ptr& a_rhs) const noexcept { return m_data  < a_rhs.m_data; }
    bool operator<=(const shared_ptr& a_rhs) const noexcept { return !(a_rhs < *this);  }
    bool operator> (const shared_ptr& a_rhs) const noexcept { return *this   > a_rhs;   }
    bool operator>=(const shared_ptr& a_rhs) const noexcept { return !(a_rhs > *this);  }
...
private:
    ...
    shared_ptr(T&&       a_data) : m_data(new ptr_data(std::forward<T>(a_data))) {}
    ...
    struct ptr_data {
        ...
        ptr_data(T&&... a_value) : m_value(std::move(a_value))          {}
