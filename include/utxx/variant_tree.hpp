...
public:
    ...
    std::ostream& dump(std::ostream& out, size_t a_level=0, size_t a_tab_width=2) const {
        size_t l_max = 0;
        BOOST_FOREACH(const utxx::variant_tree::value_type& v, *this) {
        ...
        BOOST_FOREACH(const utxx::variant_tree::value_type& v, *this) {
            ...
            if (v.second.size())
                static_cast<const utxx::variant_tree&>(v.second).dump(out, a_level+1);
        }
        return out;
    }
...
protected:
        ...
        const T& a_on_update)
    {
        ...
        BOOST_FOREACH(const utxx::variant_tree::value_type& v, a_tree) {
        ...
        const T& a_on_update)
    {
        ...
        BOOST_FOREACH(utxx::variant_tree::value_type& vt, a_tree) {
            variant_tree::path_type l_path =
