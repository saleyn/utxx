...
namespace {
  // see: https://stackoverflow.com/questions/7890194/optimized-itoa-function
  static uint16_t const s_str100p[100] = {
...
/// Convert a number to a string, given the buffer space
///
/// @return pointer to the beginning of the string; "result" contains ptr to the
/// end.
///
/// NOTE: This function is about 20% faster than itoa(), but slower than fast_itoa()
template <typename T>
char* itoa(T val, char*& buf)
{
    ...
    for(; val >= 100; p += 2) {
        ...
        val /= 100;
        memcpy(p, s_str100p + (old - (val * 100)), sizeof(uint16_t));
    }

    memcpy(p, s_str100p + val, sizeof(uint16_t));
