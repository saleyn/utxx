// See: http://probablydance.com/2013/01/13/a-faster-implementation-of-stdfunction
/*
...
#ifdef _MSC_VER
#define UTXX_FUNC_NOEXCEPT
#define UTXX_FUNC_TEMPLATE_NOEXCEPT(FUNCTOR, ALLOCATOR)
#define UTXX_FUNC_CONSTEXPR const
#else
#define UTXX_FUNC_NOEXCEPT noexcept
#define UTXX_FUNC_TEMPLATE_NOEXCEPT(FUNCTOR, ALLOCATOR) noexcept(detail::is_inplace_allocated<FUNCTOR, ALLOCATOR>::value)
#define UTXX_FUNC_CONSTEXPR constexpr
#endif
#ifdef __GNUC__
...
#define UTXX_FUNC_MOVE(value) static_cast<typename std::remove_reference<decltype(value)>::type &&>(value)
#define UTXX_FUNC_FORWARD(type, value) static_cast<type &&>(value)

namespace utxx
{
#ifndef UTXX_FUNC_NO_EXCEPTIONS
    struct bad_function_call : std::exception
    {
        const char * what() const UTXX_FUNC_NOEXCEPT override
        {
...
#   endif
    ...
    T to_functor(T && func)
    {
        return UTXX_FUNC_FORWARD(T, func);
    }
...
#       endif
    ...
    struct manager_storage_type
    {
        ...
        Allocator & get_allocator() UTXX_FUNC_NOEXCEPT
        {
            return reinterpret_cast<Allocator &>(manager);
        }
        template<typename Allocator>
        const Allocator & get_allocator() const UTXX_FUNC_NOEXCEPT
        {
    ...
    struct function_manager_inplace_specialization
    {
        ...
        static Result call(const functor_padding & storage, Arguments... arguments)
        {
            ...
            return const_cast<T &>(reinterpret_cast<const T &>(storage))(UTXX_FUNC_FORWARD(Arguments, arguments)...);
        }

        static void store_functor(manager_storage_type & storage, T to_store)
        {
            new (&get_functor_ref(storage)) T(UTXX_FUNC_FORWARD(T, to_store));
        }
        static void move_functor(manager_storage_type & lhs, manager_storage_type && rhs) UTXX_FUNC_NOEXCEPT
        {
            new (&get_functor_ref(lhs)) T(UTXX_FUNC_MOVE(get_functor_ref(rhs)));
        }
        static void destroy_functor(Allocator &, manager_storage_type & storage) UTXX_FUNC_NOEXCEPT
        {
            get_functor_ref(storage).~T();
        }
        static T & get_functor_ref(const manager_storage_type & storage) UTXX_FUNC_NOEXCEPT
        {
    ...
    struct function_manager_inplace_specialization<T, Allocator, typename std::enable_if<!is_inplace_allocated<T, Allocator>::value>::type>
    {
        ...
        static Result call(const functor_padding & storage, Arguments... arguments)
        {
            ...
            // in debug when nothing gets inlined
            return (*reinterpret_cast<const typename std::allocator_traits<Allocator>::pointer &>(storage))(UTXX_FUNC_FORWARD(Arguments, arguments)...);
        }
        ...
        static void store_functor(manager_storage_type & self, T to_store)
        {
            ...
            std::allocator_traits<Allocator>::construct(allocator, *ptr, UTXX_FUNC_FORWARD(T, to_store));
        }
        static void move_functor(manager_storage_type & lhs, manager_storage_type && rhs) UTXX_FUNC_NOEXCEPT
        {
            ...
            new (&get_functor_ptr_ref(lhs)) typename std::allocator_traits<Allocator>::pointer(UTXX_FUNC_MOVE(get_functor_ptr_ref(rhs)));
            // this next assignment makes the destroy function easier
            get_functor_ptr_ref(rhs) = nullptr;
        }
        static void destroy_functor(Allocator & allocator, manager_storage_type & storage) UTXX_FUNC_NOEXCEPT
        {
        ...
        static typename std::allocator_traits<Allocator>::pointer & get_functor_ptr_ref(manager_storage_type & storage) UTXX_FUNC_NOEXCEPT
        {
            return reinterpret_cast<typename std::allocator_traits<Allocator>::pointer &>(storage.functor);
        }
        static const typename std::allocator_traits<Allocator>::pointer & get_functor_ptr_ref(const manager_storage_type & storage) UTXX_FUNC_NOEXCEPT
        {
    ...
    {
        new (&storage.get_allocator<Allocator>()) Allocator(UTXX_FUNC_MOVE(allocator));
    ...
    {
        template<typename T, typename Allocator>
        inline static UTXX_FUNC_CONSTEXPR function_manager create_default_manager()
        {
#           ifdef _MSC_VER
...
#       endif
        ...
        static void templated_call_move_and_destroy(manager_storage_type & lhs, manager_storage_type && rhs)
        {
            ...
            specialization::move_functor(lhs, UTXX_FUNC_MOVE(rhs));
            specialization::destroy_functor(rhs.get_allocator<Allocator>(), rhs);
            create_manager<T, Allocator>(lhs, UTXX_FUNC_MOVE(rhs.get_allocator<Allocator>()));
...
#       endif
    ...
    {
        static UTXX_FUNC_CONSTEXPR function_manager default_manager = function_manager::create_default_manager<T, Allocator>();
...
public:
    ...
    function(std::nullptr_t) UTXX_FUNC_NOEXCEPT
    {
        initialize_empty();
    }
    function(function && other) UTXX_FUNC_NOEXCEPT
    {
        initialize_empty();
        swap(other);
    }
    ...
    template<typename T>
    function(T functor,
            typename std::enable_if<detail::is_valid_function_argument<T, Result (Arguments...)>::value, detail::empty_struct>::type = detail::empty_struct()) UTXX_FUNC_TEMPLATE_NOEXCEPT(T, std::allocator<typename detail::functor_type<T>::type>)
    {
        ...
        else
        {
            ...
            initialize(detail::to_functor(UTXX_FUNC_FORWARD(T, functor)), std::allocator<functor_type>());
        }
    }
    template<typename Allocator>
    ...
    function(std::allocator_arg_t, const Allocator & allocator, T functor,
            ...
            UTXX_FUNC_TEMPLATE_NOEXCEPT(T, Allocator)
    {
        if (detail::is_null(functor))
        {
            initialize_empty();
        ...
        {
            initialize(detail::to_functor(UTXX_FUNC_FORWARD(T, functor)), Allocator(allocator));
        }
    }
    template<typename Allocator>
    ...
    template<typename Allocator>
    function(std::allocator_arg_t, const Allocator &, function && other) UTXX_FUNC_NOEXCEPT
    {
        // ignore the allocator because I don't allocate
        initialize_empty();
        swap(other);
    ...
    function & operator=(function other) UTXX_FUNC_NOEXCEPT
    {
        swap(other);
        return *this;
    }
    ...
    {
        return call(manager_storage.functor, UTXX_FUNC_FORWARD(Arguments, arguments)...);
    }

    template<typename T, typename Allocator>
    void assign(T && functor, const Allocator & allocator) UTXX_FUNC_TEMPLATE_NOEXCEPT(T, Allocator)
    {
...
private:
    ...
    void initialize(T functor, Allocator && allocator)
    {
        ...
        detail::create_manager<T, Allocator>(manager_storage, UTXX_FUNC_FORWARD(Allocator, allocator));
        detail::function_manager_inplace_specialization<T, Allocator>::store_functor(manager_storage, UTXX_FUNC_FORWARD(T, functor));
    }

    typedef Result(*Empty_Function_Type)(Arguments...);
    void initialize_empty() UTXX_FUNC_NOEXCEPT
    {
...
template<typename T>
bool operator==(std::nullptr_t, const function<T> & rhs) UTXX_FUNC_NOEXCEPT
{
    return !rhs;
}
template<typename T>
bool operator==(const function<T> & lhs, std::nullptr_t) UTXX_FUNC_NOEXCEPT
{
    return !lhs;
}
template<typename T>
...
namespace std
{
    ...
    struct uses_allocator<utxx::function<Result (Arguments...)>, Allocator>
        : std::true_type
    {};
}
...
#endif
#undef UTXX_FUNC_NOEXCEPT
#undef UTXX_FUNC_TEMPLATE_NOEXCEPT
#undef UTXX_FUNC_FORWARD
#undef UTXX_FUNC_MOVE
#undef UTXX_FUNC_CONSTEXPR
