...
//----------------------------------------------------------------------------
/// \brief Efficient buffered print stream — standalone copy with no boost
///        or utxx dependencies.
//----------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
/// String wrapper for possibly non-zero-terminated strings
//------------------------------------------------------------------------------
template <class Char = char>
struct cstr_wrap {
  cstr_wrap(const Char* a_str, size_t a_sz) : m_str(a_str), m_size(a_sz) {}
...
public:
  ...
  fixed(double a_val, int a_precision, bool a_compact = true)
      : m_value(a_val), m_info{-1, uint8_t(a_precision), ' ', a_compact, true, false}
  {}

  fixed(long a_val, int a_width, alignment a_align = RIGHT)
      : m_value(a_val), m_info{int8_t(a_width), 0, ' ', false, false, a_align == LEFT}
  {}

  fixed(int a_val, int a_width, alignment a_align = RIGHT) : fixed(long(a_val), a_width, a_align) {}
  fixed(size_t a_val, int a_width, alignment a_align = RIGHT) : fixed(long(a_val), a_width, a_align)
  {}
  ...
  inline friend StreamT& operator<<(StreamT& out, const fixed& a)
  {
    if (a.is_float()) {
      ...
      if (a.digits() > -1 && a.digits() < (int)sizeof(buf)) {
        ...
        if (likely(n >= 0)) out.write(buf, n);
      }
    } else if (a.is_left()) {
      char buf[22];
      auto end = utxx::itoa_left<long, 22, char>(static_cast<char*>(buf), a.int_value());
      ...
      for (int i = 0; i < pad; ++i) out << a.fill();
      out.write(buf, len);
    }
    return out;
  }
  ...
  alignment align() const { return m_info.is_left ? LEFT : RIGHT; }
...
//------------------------------------------------------------------------------
/// Fixed-width aligned field
//------------------------------------------------------------------------------
template <int Width, alignment Align, class T>
struct width {
  using TT = typename std::remove_cv<typename std::remove_reference<T>::type>::type;

  explicit width(T a_val, char a_pad = ' ') : m_value(a_val), m_pad(a_pad), m_precision(0)
  {
    static_assert(
        std::is_integral<TT>::value || std::is_same<const char*, TT>::value ||
            std::is_same<char*, TT>::value || std::is_same<std::string, TT>::value,
        "Must be bool, integer or string type");
  }
  ...
  void                             write(char* a_buf) const { do_write(m_value, a_buf); }
...
private:
  ...
  void padit(char* p, const char* end) const
  {
    while (p < end) *p++ = m_pad;
  }
  ...
  {
    if (Align == RIGHT)
      itoa_right<T, Width, char>(a_buf, a_value, m_pad);
    else
      itoa_left<T, Width, char>(a_buf, a_value, m_pad);
  }

  void do_write(bool a_value, char* a_buf) const { do_write(a_value ? "true" : "false", a_buf); }

  void do_write(long a_val, char* a_buf) const { int_write(a_val, a_buf); }
  void do_write(unsigned long a_val, char* a_buf) const { int_write(a_val, a_buf); }
  void do_write(int a_val, char* a_buf) const { int_write(a_val, a_buf); }
  void do_write(unsigned a_val, char* a_buf) const { int_write(a_val, a_buf); }
  ...
  void do_write(const char* a_value, size_t a_len, char* a_buf) const
  {
    ...
    if (Align == LEFT) padit(a_buf + len, a_buf + Width);
  }
  ...
  void do_write(char a_value, char* a_buf) const
  {
    ...
    *a_buf = a_value;
    if (Align == LEFT) padit(a_buf + 1, a_buf + Width);
  }
  ...
  template <typename U>
  void do_write(U, char*) const
  { throw std::runtime_error("Writing of this type is not supported"); }
};

template <int Width, alignment Align, typename T>
width<Width, Align, T> make_width(T a, char a_pad = ' ')
{ return width<Width, Align, T>(a, a_pad); }
...
class basic_buffered_print : public Alloc {
  ...
  void do_print(bool a)
  {
    static const auto f = [](char* p, bool a) {
      static const std::pair<const char*, int> s_vals[] = {
          {"false", 5},
          { "true", 4}
      };
      auto r = s_vals[a];
      memcpy(p, r.first, r.second);
      return r.second;
    };
    reserve(8);
    m_pos += f(m_pos, a);
  ...
  void do_print(std::_Setprecision s) { m_precision = s._M_n; }

public:
  explicit basic_buffered_print(const Alloc& a_alloc = Alloc())
      : Alloc(a_alloc), m_begin(m_data), m_pos(m_begin), m_end(m_begin + sizeof(m_data) - 1)
  {}

  ~basic_buffered_print() { deallocate(); }
  ...
  void        pos(const char* p)
  {
    assert(p <= m_end);
    m_pos = const_cast<char*>(p);
  }
  ...
  int printf(const char* a_fmt, ...)
  {
    va_list args;
    va_start(args, a_fmt);
    int n = this->vprintf(a_fmt, args);
    va_end(args);
    return n;
  }

  template <typename T>
  friend inline basic_buffered_print<N>& operator<<(basic_buffered_print<N>& out, T&& a)
  {
...
/// Analogous to sprintf but returns an std::string
...
std::string print_string(const char* fmt, ...)
{
  ...
  va_end(args);
  if (unlikely(n < 0)) throw std::runtime_error("print_string: format error");
  buf.resize(n);
  if (unlikely(n > SizeHint)) {
    va_start(args, fmt);
...
/// Print arguments to string (faster alternative to printf/stringstream)
template <class... Args>
...
namespace std {
template <size_t N>
inline utxx::basic_buffered_print<N>& endl(utxx::basic_buffered_print<N>& a_out)
{
  a_out << '\n';
  return a_out;
}
...
template <size_t N>
inline utxx::basic_buffered_print<N>& ends(utxx::basic_buffered_print<N>& a_out)
{
  a_out << '\0';
  return a_out;
}
...
template <size_t N>
inline utxx::basic_buffered_print<N>& flush(utxx::basic_buffered_print<N>& a_out)
{ return a_out; }
} // namespace std
