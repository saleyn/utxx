...
void logger_impl_mgr::register_impl(
    const char* config_name, boost::function<logger_impl*(void)>& factory)
{
...
boost::function<logger_impl*(void)>*
...
void logger::init(const char* filename, init_file_type type)
{
    ...
    switch (type) {
        ...
        case JSON_FILE: read_json(filename, pt); break;
        case XML_FILE:  read_xml (filename, pt); break;
    }
    init(pt);
}

void logger::init(const ptree& config)
{
    finalize();
    ...
    try {
            ...
            ++it)
        {
            ...
            if (config.get_child_optional(path)) {
                ...
                boost::function< logger_impl* (void) >& f = it->second;
                std::auto_ptr<logger_impl> impl( f() );
