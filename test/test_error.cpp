...
BOOST_AUTO_TEST_CASE( test_error )
{
    ...
    BOOST_REQUIRE_EQUAL("a bc", utxx::runtime_error("a", "b", "c").str());

    { auto e = utxx::runtime_error("a");               BOOST_REQUIRE_EQUAL("a ",   e.str()); }
    { auto e = utxx::runtime_error("a") << "b";        BOOST_REQUIRE_EQUAL("a b",  e.str()); }
    { auto e = utxx::runtime_error("a") << "b" << "c"; BOOST_REQUIRE_EQUAL("a bc", e.str()); }

    BOOST_REQUIRE_EQUAL("a: Success",    utxx::io_error(0, "a").str());
    BOOST_REQUIRE_EQUAL("a b: Success",  utxx::io_error(0, "a", " b").str());
    BOOST_REQUIRE_EQUAL("a bc: Success", utxx::io_error(0, "a", " b", "c").str());
