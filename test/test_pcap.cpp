...
static int cmp(const uint8_t* a_exp, const char* a_got, size_t a_sz) {
    int  res = memcmp(a_exp, a_got, a_sz);
    if (!res)  return res;

    auto hex = [=](const uint8_t* p) {
        const uint8_t* e = p + a_sz;
        char buf[1024];
        for (int i=0, j=0; p != e; ++p, ++i)
            j+=sprintf(buf+j, "%s0x%02x%s", i ? "," : "", *p,
                       (i && ((i+1) % 12 == 0)) ? "\n            ":"");
        return std::string(buf);
    };

    uint i;
    for (i=0; i < a_sz && uint8_t(a_got[i]) == a_exp[i]; ++i);
    BOOST_TEST_MESSAGE(
        "  Mismatch at " << i << " (0x" << std::hex
        << (int)*(uint8_t*)(a_got+i)    << std::dec
        << "):\n  Exp:\n             "  << hex(a_exp)
        <<   "\n  Got:\n             "  << hex((const uint8_t*)a_got));
    return res;
}
...
BOOST_AUTO_TEST_CASE( test_pcap_writer )
{
    ...
    // Write UDP packet with ETHERNET header
    {
        ...
        static const uint8_t s_exp[] = {
            ...
            ,0x54,0xa6,0x0a,0xa5,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x34
    ...
    // Write TCP packet with ETHERNET header
    {
        ...
        static const uint8_t s_exp[] = {
             0xa1,0xb2,0xc3,0xd4,0x00,0x02,0x00,0x04,0x00,0x00,0x00,0x00
            ,0x00,0x00,0x00,0x00,0x00,0x00,0xff,0xff,0x00,0x00,0x00,0x01
            ,0x54,0xa6,0x0a,0xa5,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x40
    ...
    // Write UDP packet without ETHERNET header
    {
        ...
        static const uint8_t s_exp[] = {
            ...
            ,0x54,0xa6,0x0a,0xa5,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x26
            ,0x00,0x00,0x00,0x26,0x45,0x00,0x00,0x26,0x00,0x00,0x00,0x00
    ...
    // Write TCP packet without ETHERNET header
    {
        ...
        static const uint8_t s_exp[] = {
             0xa1,0xb2,0xc3,0xd4,0x00,0x02,0x00,0x04,0x00,0x00,0x00,0x00
            ,0x00,0x00,0x00,0x00,0x00,0x00,0xff,0xff,0x00,0x00,0x00,0x65
            ,0x54,0xa6,0x0a,0xa5,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x32
            ,0x00,0x00,0x00,0x32,0x45,0x00,0x00,0x32,0x00,0x00,0x00,0x00
