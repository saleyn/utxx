...
BOOST_AUTO_TEST_CASE( test_pcap_writer )
{
    ...
    // Write UDP packet without ETHERNET header
    {
        ...
        if (res) {
            uint i;
            for (i=0; i < length(s_exp) && uint8_t(s.c_str()[i]) == s_exp[i]; ++i);
            BOOST_TEST_MESSAGE(
                "  Mismatch at "  << i << " (" << std::hex <<
                (int)*(uint8_t*)(s.c_str()+i)  <<
                std::dec << "):\n  " <<
                to_bin_string((const char*)s_exp, s.size(), true, false) <<
                "\n  "   <<
                to_bin_string(s.c_str(),          s.size(), true, false));
        }
        BOOST_REQUIRE(res == 0);
    }
